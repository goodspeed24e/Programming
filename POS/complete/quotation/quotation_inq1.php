...
<?php 
	...
	$lista=ceil($total/10);
?>
<!----傳給quotation_inq2.php,執行搜尋---------->
<form name="form1" method="post" action="quotation/quotation_inq2.php">
...
<?php
	...
	for($i = 1;$i<=$lista;$i++) 
	{
		echo "  <a href=index.php?Select=156&page=$i>$i</a>|";
	}
	  ...
	  $res = mysql_query($SQLStr);
       while (list($quwkno, $quwkprod, $quwkcust, $quwktype, $quwkpric, $quwkqty, $quwksdate)=mysql_fetch_row($res))
        ...
        //--------------- 抓取報價資料表中資料並呈現---------------
			...
			echo "<td align='center'><a href='quotation/quotation_inq2.php?quwkno=" . $quwkno . "&quwkprod=" .$quwkprod. "&quwkcust=" .$quwkcust. "'>" . $quwkprod . "</a></td>\n";
