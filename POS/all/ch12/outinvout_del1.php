...
<?php
	...
	$lista=ceil($total/10);
?>
<form name="form1" method="post" action="outinv/outinvout_del2.php">
...
//--------------頁數選擇呈現及連接---------------
	...
	for($i = 1;$i<=$lista;$i++) 
	{
		echo "  <a href=index.php?Select=404&page=$i>$i</a>|";
	}
...
<?php
      ...
      while (list($outicode, $outino, $outiprodno, $outidate, $outiremark, $outicustno, $outiprodname, $outiprodqty, $outiunit, $outipric, $outiamt)=mysql_fetch_row($res))
		...
		//------------- 抓取銷貨資料表中資料並呈現於網頁上-------------------------
			...
			echo "<td align='center'><a href='outinv/outinvout_del2.php?outino=" . $outino . "&outiprodno=" . $outiprodno . "'>" . $outiprodno . "</a></td>\n";
