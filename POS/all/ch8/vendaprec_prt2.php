...
// -------------取出所有進貨單的資料---------------
	...
	$SQLStr = "SELECT transaction.* FROM transaction WHERE (trandate >='$trandate1' and trandate<='$trandate2' and tranvendno='$tranvendno1') ";	
  ...
  <table width="596" border="0" align="center" cellspacing="0" cellpadding="0" >
    <tr > 
      <td width="603" align="left">
		 <?php
//--------------頁數選擇呈現及連接---------------
     echo "廠商應付帳款表&nbsp;&nbsp;<br><font size='-1'>廠商編號:".$tranvendno1."&nbsp;日期:".$trandate1."至".$trandate2."&nbsp; 第&nbsp";
	for($i = 1;$i<=$lista;$i++) 
	{
...
//--------------選擇頁數進貨單資料呈現及連接頁面功能-----------
	  ...
	  $first=30*($page - 1);
	  $SQLStr="select * from transaction WHERE (trandate >='$trandate1' and trandate<='$trandate2' and tranvendno='$tranvendno1') limit $first , 30";
	  $res = mysql_query($SQLStr);	 
