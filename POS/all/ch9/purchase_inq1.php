...
<?php 
	...
	$lista=ceil($total/10);
?>
<!----傳給purchase_inq2.php,執行搜尋---------->
<form name="form1" method="post" action="purchase/purchase_inq2.php">
...
<?php
	...
	for($i = 1;$i<=$lista;$i++) 
	{
		echo "  <a href=index.php?Select=256&page=$i>$i</a>|";
	}
	  ...
	  $res = mysql_query($SQLStr);	
       while (list($purchtranno, $purchprodno, $purchprodqty, $purchinqty, $purchpric)=mysql_fetch_row($res))
        ...
        //------------ 抓取採購資料表中資料並呈現----------------------------------
			...
			echo "<td align='center'><a href='purchase/purchase_inq2.php?purchprodno=" . $purchprodno . "&purchtranno=" .$purchtranno. "'>" . $purchprodno . "</a></td>\n";
