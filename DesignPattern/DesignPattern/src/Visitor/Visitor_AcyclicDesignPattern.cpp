...
/*virtual*/ void This::accept( Visitor& v ) {
   ...
   if (tv) 
	   ...
	   cout << "the visitor was not accepted\n";
}
 ...
 * 
 * Problem. In GOF visitor, Element depends on Visitor, Visitor depends
 * on all Element derivatives, and Element derivatives depend on Element;
 * this is cyclic dependency. Additionally, adding an element derivative
 * requires the entire Visitor hierarchy to change. "These problems can be 
 * solved by using multiple inheritance and dynamic_cast()."
 *
 * Solution:
 *	Element derived class are only coupled to Visitor base class. Visitor derived
 * classes are only coupled to the Element derived classes that they choose to 
 * be coupled to. If a new Element derived class is added, Vistor derived classes
 * can update themselves if, and when, they choose.
 * \remarks
